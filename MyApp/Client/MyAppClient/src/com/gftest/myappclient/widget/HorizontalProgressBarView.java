...
public class HorizontalProgressBarView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawRect(startX, startY, ((float) progress / max) * mwidth, mhight, paint);
		// 绘制内部线条
		paint.setStrokeWidth(2);
		paint.setColor(Color.YELLOW);
		canvas.drawLine(startX, startY + (mhight - startY) / 4 * 1, ((float) progress / max) * mwidth, startY + (mhight - startY) / 4 * 1, paint);
		paint.setColor(Color.RED);
		canvas.drawLine(startX, startY + (mhight - startY) / 4 * 2, ((float) progress / max) * mwidth, startY + (mhight - startY) / 4 * 2, paint);
		paint.setColor(Color.WHITE);
		canvas.drawLine(startX, startY + (mhight - startY) / 4 * 3, ((float) progress / max) * mwidth, startY + (mhight - startY) / 4 * 3, paint);
		// 绘制下标进度数字
		if (progress < (max / 3)) {
		...
		canvas.drawText(progress + "%", ((float) progress / max) * mwidth - 20, mhight + 30, paint);
	}
}
