...
public class QuanDaoImpl {
	...
	public boolean addQuan(QuanEntity quanEntity) {
		...
		if (quanEntity != null) {
			...
			contentValues.put(Constants.TABLE_QUAN_COLUMN3, quanEntity.getOrderId());
	...
	public List<QuanEntity> getQuanList(String selection, String[] selectionArgs) {
		...
		Cursor cursor = database.query(Constants.TABLE_NAME, null, selection, selectionArgs, null, null, Constants.TABLE_QUAN_COLUMN3 + " asc");
		if (cursor.moveToFirst()) {
			...
			do {
				...
				quanEntity.setOrderId(cursor.getInt(cursor.getColumnIndex(Constants.TABLE_QUAN_COLUMN3)));
